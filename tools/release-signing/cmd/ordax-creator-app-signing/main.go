...
func validateManifest(data []byte) (manifest, error) {
	...
	if m.Schema != manifestSchema || m.Purpose != purpose || m.SourceRepository != repository || m.CreatedFromRecipe != recipe {
		return manifest{}, errors.New("Creator app manifest identity/purpose is not canonical")
	}
	if !commitPattern.MatchString(m.SourceCommit) || !versionPattern.MatchString(m.Version) || m.ReleaseSequence <= 0 {
		return manifest{}, errors.New("Creator app source/version/sequence is invalid")
	}
	if m.Artifact.Name != artifactName || !shaPattern.MatchString(m.Artifact.SHA256) || m.Artifact.Size <= 0 || m.Artifact.Size > maxArtifact {
		return manifest{}, errors.New("Creator app artifact binding is invalid")
	}
	parsed, err := url.Parse(m.Artifact.URL)
	expected, _ := url.Parse(artifactURL)
	if err != nil || parsed.Scheme != "https" || parsed.Host != "github.com" || parsed.User != nil || parsed.Fragment != "" || parsed.RawQuery != "" || parsed.Path != expected.Path {
		return manifest{}, errors.New("Creator app artifact URL is outside canonical creator-app release")
	}
	return m, nil
}
...
func signApp(manifestPath, privatePath, trustPath, outputPath, keyID string) (manifest, error) {
...
func main() {
	flags := flag.NewFlagSet("ordax-creator-app-signing", flag.ExitOnError)
	manifestPath := flags.String("manifest", "", "canonical Creator app manifest")
	...
	outputPath := flags.String("out", "", "new signed Creator app envelope")
	...
	if flags.NArg() != 0 || *manifestPath == "" || *privatePath == "" || *trustPath == "" || *outputPath == "" || *keyID == "" {
		fmt.Fprintln(os.Stderr, "usage: ordax-creator-app-signing --manifest FILE --private-key FILE --trust FILE --out FILE --key-id ID")
		os.Exit(2)
	}
	m, err := signApp(*manifestPath, *privatePath, *trustPath, *outputPath, *keyID)
	if err != nil {
		fmt.Fprintln(os.Stderr, "ordax-creator-app-signing: ERROR:", err)
		os.Exit(1)
	}
	fmt.Printf("CREATOR_APP_ENVELOPE_SIGNED=YES\nSOURCE_COMMIT=%s\nVERSION=%s\nRELEASE_SEQUENCE=%d\nPURPOSE=%s\nPRIVATE_KEY_PRINTED=NO\n", m.SourceCommit, m.Version, m.ReleaseSequence, purpose)
}
