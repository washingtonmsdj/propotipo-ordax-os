...
- [ ] Foco, scroll e formulário não são reiniciados por toda atualização de snapshot. **Parcial avançado:** Ajustes → Rede e o painel rápido de Wi-Fi preservam scroll, foco, seleção e senha digitada somente em memória durante repaint causado por polling/status. O painel rápido limpa o rascunho ao fechar, trocar de rede, conectar ou destruir o owner; nenhuma credencial entra em store/sync. Aplicar o mesmo princípio aos demais formulários/owners antes de marcar concluído.
- [ ] Estado ausente, indisponível, offline e antigo são distintos. **Parcial avançado:** Ajustes → Rede, Sistema → métricas, a barra de rede e o painel rápido de Wi-Fi preservam a última leitura válida após falha e a identificam como antiga, separando-a de carregamento inicial, capacidade indisponível e estado offline. A barra neutraliza o ícone quando a leitura é antiga em vez de continuar sugerindo conectividade atual. O horário exibido é o momento em que a Surface recebeu a leitura, não um timestamp inventado do host; aplicar a mesma semântica aos demais owners observacionais, especialmente energia/bateria, antes de marcar concluído.
