...
| Arquivos | Adapter nativo delimitado, listar pastas/arquivos, abrir pastas, breadcrumbs, criar pasta, atualizar listagem. | Abrir conteúdo de arquivos, seleção, renomear, copiar/mover, importação/exportação, busca, favoritos e lixeira. |
...
1. `system/contracts/file-space.mjs` evoluiu para `ordax.file-space/3`: além de `list()`, `createDirectory()` e `readTextFile()`, o Native oferece `renameEntry()` com operação atômica sem sobrescrita dentro da pasta autorizada. A visualização continua somente leitura, UTF-8 e limitada a 256 KB. Entradas continuam com `name`, `kind` e `size`; ainda não há data de modificação, copiar, mover, excluir/lixeira ou abertura por associação de app.
...
7. As métricas disponíveis são tempo ligado, memória total/disponível e capacidade total/livre do espaço do usuário. Não há nesse contrato inventário de CPU/GPU, bateria, temperatura, volumes físicos ou categorias de armazenamento.
...
9. “Configurações” ainda aparece em títulos internos, enquanto o rail usa “Ajustes”. Padronizar o rótulo visível para **Ajustes**, preservando o ID estável `settings`.
10. Um runtime em execução não prova que está na versão remota mais recente. Revisar rótulos como “Atualizado” baseados apenas em `running`.
