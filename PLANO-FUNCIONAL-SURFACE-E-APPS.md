...
| Arquivos | Adapter nativo delimitado, navegação/breadcrumbs, seleção, preview UTF-8, criar pasta, renomear, duplicar/copiar, mover, importar/exportar, busca local, ordenação e metadado Modificado real. | Favoritos, Recentes, lixeira, múltiplos itens/diretórios e “Abrir com” para apps compatíveis ainda faltam. |
...
1. `system/contracts/file-space.mjs` evoluiu para `ordax.file-space/10`: além de `list()`, `createDirectory()`, `readTextFile()`, `renameEntry()` e `copyFile()`, o Native oferece `moveEntry()` para arquivo ou pasta entre diretórios da mesma raiz autorizada, com no-clobber atômico via `renameat2`, bloqueio de symlink, rejeição de mover pasta para dentro dela mesma e movimento de arquivo regular entre volumes por copy+verify+remove, mantendo rejeição explícita para pasta cross-device. `copyFile()` copia arquivo regular entre quaisquer diretórios autorizados da raiz, inclusive entre filesystems quando o host os expõe sob essa raiz, com limite de 64 MiB, origem e destino explícitos, criação exclusiva do destino, verificação de mudança concorrente e limpeza de cópia parcial em falha. A ação **Duplicar** reutiliza a mesma operação apontando origem e destino para a pasta atual, enquanto **Copiar para…** usa o fluxo único de navegação até a pasta de destino. A visualização continua somente leitura, UTF-8 e limitada a 256 KB. Entradas trazem `name`, `kind`, `size` e `modifiedAt` real em epoch milliseconds obtido do filesystem. O Native também oferece exportação explícita de arquivo regular para download mediado pelo navegador e importação explícita a partir do seletor de arquivo do navegador. Ambas são limitadas a 64 MiB e same-origin; exportação bloqueia symlink e detecta mudança durante a leitura, enquanto importação cria destino com no-clobber, streaming, `fsync` e remove qualquer destino parcial em falha. Cópia de arquivo entre pastas e movimento de arquivo regular entre volumes estão implementados; diretórios/múltiplos itens, movimento cross-device de pastas, excluir/lixeira e abertura por associação de app continuam pendentes.
...
**Estado:** NOVO/P2. **Aceite:** limpar recentes preserva todos os arquivos e informa que somente o histórico foi apagado.
...
**Estado:** PARCIAL/P1. Contraste **Padrão/Reforçado** e Movimento **Padrão/Reduzido** existem como preferências reais da Surface, com validação, persistência e aplicação imediata nos dois temas. Movimento reduzido também continua respeitando `prefers-reduced-motion` do ambiente: a escolha explícita pode reduzir mais, nunca força animação quando o ambiente pediu redução. Tamanho de texto, restauração conjunta da seção, atalhos e recursos de cursor/leitor continuam pendentes; recursos do sistema dependem de adapter próprio.
...
| E3 / P1 | Ajustes: Aparência e acessibilidade da Surface; retirar diagnóstico técnico da tela comum. | E1, catálogo/store. | **Parcial:** Aparência, Acessibilidade e Rede são destinos canônicos; tema, contraste e movimento são preferências reais/persistidas; a listagem técnica de capacidades foi removida de Ajustes. Tamanho de texto e demais preferências ainda faltam. |
...
| SET-03 | Aumentar texto/reduzir movimento. | **Parcial:** movimento reduzido é preferência real e elimina animações/transições não essenciais; tamanho de texto ainda falta. Interface deve continuar operável em zoom do navegador enquanto a escala própria não existe. |
