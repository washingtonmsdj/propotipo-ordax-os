...
- [ ] Foco, scroll e formulário não são reiniciados por toda atualização de snapshot. **Parcial avançado:** Ajustes → Rede, painel rápido de Wi-Fi e Arquivos preservam interação transitória durante repaint. Arquivos conserva scroll da janela/lista/prévia, foco e seleção de busca/criar/renomear/copiar quando o caminho não mudou; navegação para outro caminho continua iniciando contexto visual novo. Rascunhos permanecem somente em memória. Aplicar a mesma disciplina aos owners observacionais sem formulário que ainda recompõem seções inteiras antes de marcar concluído.
