...
The host-base heartbeat forwards the updater's `checkedAt` value as
`supervisorCheckedAt` and also reports the filesystem mtime of the updater state as
`supervisorStateEpoch`. The mtime is the robust liveness signal: the supervisor
rewrites that state file as it checks for updates, while the base telemetry process
has an independent lifetime. A fresh relay `last_seen_at` paired with a stale
`supervisorStateEpoch` therefore identifies a stalled supervisor without adding a
second supervisor timer or control path.
