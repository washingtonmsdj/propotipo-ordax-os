...
class NativeNetworkStatusTests(unittest.TestCase):
    ...
    def test_contract_adapter_composition_and_shared_ui_stay_separated(self):
        ...
        self.assertIn("Estado real do host e gerenciamento Wi-Fi", controls)
        self.assertIn("O gerenciamento de Wi-Fi não está disponível neste ambiente.", controls)
        self.assertIn("networkLastSuccessAt", controls)
        self.assertIn("networkManagementLastSuccessAt", controls)
        self.assertIn("Dados de interface antigos", controls)
        self.assertIn("Dados de Wi-Fi antigos", controls)
