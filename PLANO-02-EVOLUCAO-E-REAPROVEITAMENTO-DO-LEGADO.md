...
2. Renomear arquivo/pasta, com validação consistente no cliente e no owner da operação. **Estado atual:** implementado no `ordax.file-space/10`, com rename atômico sem sobrescrita, rejeição de symlink/colisão, formulário contextual e regressões de preservação de origem/destino.
...
**MVP:** criar/abrir projeto com nome, pasta escolhida ou referência lógica, última atividade e apps associados; reabrir sem perder vínculo. Começar com projeto genérico ou de desenvolvimento, não com dez verticais. Pasta inacessível deve produzir “local indisponível”, sem apagar projeto.
...
| Arquivos → Projetos | Contexto de trabalho e ligação com conteúdo/apps, quando C04 existir. | Instalação de app requerido vai ao owner de aplicativos. |
...
| F4 | Contexto local: projeto pequeno, recentes e retomada. | F2, C04–C06. | **Parcial:** Recentes locais de Arquivos está implementado com histórico limitado/deduplicado, registro somente após abertura validada, persistência `device`/`session` explícita e remoção/limpeza não destrutivas. Arquivos também retoma após reload a última pasta listada com sucesso usando o `target` já persistido no workspace, com fallback seguro para a última pasta válida ou `/`. Projeto pequeno e retomada contextual de conteúdo/atividade além da pasta atual ainda precisam ser entregues antes de encerrar F4. |
...
F4 começou por dois recortes locais no app Arquivos. Recentes usa uma fonte única que registra somente arquivos realmente abertos com sucesso, mantém retenção limitada e permite remover/limpar referências sem apagar conteúdo. A navegação também persiste no workspace somente a última pasta listada e validada com sucesso e a retoma após reload, sem criar outro store; target inválido volta para a última pasta válida ou `/`. Isso ainda não encerra F4: projeto pequeno e retomada contextual de conteúdo/atividade continuam como próximos recortes.
