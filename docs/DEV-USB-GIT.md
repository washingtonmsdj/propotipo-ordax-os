...
Activation remains deliberately split by layer. The A/B staging primitives for kernel/initramfs already target an inactive boot slot, but the development-channel `dev-base-ready-sha` handoff is **not yet wired to ESP staging/one-shot arming**. Therefore this change does not write or arm boot media automatically. The development rootfs is materialized under `/ordax/dev-base/versions/<commit>` and its selector is ready to run only when a later authorized one-shot A/B candidate boot supplies matching `ordax.base_candidate=<commit>` and `ordax.base_slot=a|b`. The seed `/ordax/dev-base` always starts first; `ordax-dev-init` then pivots into the versioned root while preserving `/state`, `/workspace`, `/home`, `/proc`, `/sys` and `/dev`. A candidate that cannot select its exact rootfs never reaches the normal OrdaX runtime and therefore cannot satisfy the existing base + Surface health promotion gate. The selected rootfs is remembered per A/B slot, so a failed candidate falls back to the previous slot/rootfs and a legacy slot without a mapping keeps the known-good seed Base. Physical broken-candidate proof remains pending. The UEFI fallback loader remains the final explicitly physical self-update gap.
